...
To support modern mobile recordings (often 48 kHz), the system aligns audio samples back to the native **44.1 kHz** protocol rate:
- **Resampling**: Offline WAV processing employs Fast Fourier Transform (**FFT**) frequency-domain resampling. Live Microphone feeds employ **Linear Interpolation** to handle chunk arrivals efficiently.
- **Noise Filtering**: The system includes **FFT bandpass filtering (brick-wall)** to remove background rumble and ensure accurate pixel amplitude mapping.
